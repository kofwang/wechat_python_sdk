...
class WXAPIError(WXBaseException):
    """
    ...
    def __init__(self, error_code, error):
        ...
        self.error = error
        super(WXAPIError, self).__init__(self, error)

    def __str__(self):
        return 'WXAPIError: %s: %s' % (self.error_code, self.error)
...
def _http_call(_url, method, authorization, **kw):
    ...
    if not isinstance(body, basestring):
        if 'error' in body:
            raise WXAPIError(body.error, getattr(body, 'error_description', ''))
        if 'ret' in body and body.ret != 0:
            raise WXAPIError(body.ret, getattr(body, 'msg', ''))
    return body
    ...
    def __getattr__(self, attr):
        def wrap(**kw):
            if self.client.is_expires():
                raise WXAPIError('100015', 'access token is revoked')
...
class APIClient(object):
    ...
    def __init__(self, app_id, app_key, redirect_uri=None, response_type='code'):
        ...
        self.auth_url = 'https://%s/sns/oauth2/' % WX_DOMAIN
        self.api_url = 'https://%s/sns/' % WX_DOMAIN
    ...
    def get_authorize_url(self, redirect_uri=None, display=None, response_type='code', endpoint='authorize', 
        ...
        if not redirect:
            raise WXBaseException('Redirect uri is needed.')
    ...
    def request_access_token(self, code, grant_type='authorization_code', endpoint='access_token'):
        """
    ...
    def set_unionid(self, unionid):
        self.unionid = unionid

    def get_userinfo(self):
        """
        https://api.weixin.qq.com/sns/userinfo?access_token=YOUR_ACCESS_TOKEN&openid=OPENID&lang=zh_CN
        """
        if self.is_expires():
            raise WXBaseException('Your muse set a correct access token to request an openid')
        ret = _http_get('%s%s' % (self.api_url, 'userinfo'), 
